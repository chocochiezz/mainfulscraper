...
{
    public class PlaceUserImageController : BaseController
    {
		[HttpPost]
        public override JsonResultEntity Get([FromBody] DBParamEntity dbParamEntity)
        {
            ...
            try
            {
                var result = placeuserimageBL.GetAll(dbParamEntity);
                ...
                var dataFound = placeuserimageBL.GetTotalRows(dbParamEntity);
		...
		public JsonResultEntity GetById(int id)
        {
            ...
            try
            {
                var result = placeuserimageBL.GetById(id);
        ...
        {
            var placeuserimageBL = new PlaceUserImageBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = placeuserimageBL.DeleteById(id);
                if (result.HasWarning())
                {
                    response.Message = String.Join(",", result.Warning);
                    return response;
