...
namespace Mainful.AdminUI.Controllers
{
    public class PromoController : BaseController
    {
        ...
        public override JsonResultEntity Get([FromBody] DBParamEntity dbParamEntity)
        {
            ...
            try
            {
                var result = promoBL.GetAll(dbParamEntity);
                ...
                var dataFound = promoBL.GetTotalRows(dbParamEntity);
        ...
        [HttpPost]
        public JsonResultEntity Update([FromBody] PromoEntity promoEntity)
        {
            PromoBL promoBL = new PromoBL();
            JsonResultEntity response = new JsonResultEntity();
            ...
            try
            {
                var result = promoBL.Update(promoEntity);
        ...
        {
            var promoBL = new PromoBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = promoBL.DeleteById(id);
                if (result.HasWarning())
                {
                    response.Message = String.Join(",", result.Warning);
                    return response;
