...
{
    public class PromoImageController : BaseController
    {
		[HttpPost]
        public override JsonResultEntity Get([FromBody] DBParamEntity dbParamEntity)
        {
            ...
            try
            {
                var result = promoimageBL.GetAll(dbParamEntity);
                ...
                var dataFound = promoimageBL.GetTotalRows(dbParamEntity);
		...
		public JsonResultEntity GetById(int id)
        {
            ...
            try
            {
                var result = promoimageBL.GetById(id);
        ...
        [HttpPost]
        public JsonResultEntity Update([FromBody] PromoImageEntity promoimageEntity)
        {
            PromoImageBL promoimageBL = new PromoImageBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = promoimageBL.Update(promoimageEntity);
        ...
        {
            var promoimageBL = new PromoImageBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = promoimageBL.DeleteById(id);
                if (result.HasWarning())
                {
                    response.Message = String.Join(",", result.Warning);
                    return response;
