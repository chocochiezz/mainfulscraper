...
{
    public class SchedulerSessionController : BaseController
    {
		[HttpPost]
        public override JsonResultEntity Get([FromBody] DBParamEntity dbParamEntity)
        {
            ...
            try
            {
                var result = schedulersessionBL.GetAll(dbParamEntity);
                ...
                var dataFound = schedulersessionBL.GetTotalRows(dbParamEntity);
		...
		public JsonResultEntity GetById(int id)
        {
            ...
            try
            {
                var result = schedulersessionBL.GetById(id);
        ...
        [HttpPost]
        public JsonResultEntity Update([FromBody] SchedulerSessionEntity schedulersessionEntity)
        {
            SchedulerSessionBL schedulersessionBL = new SchedulerSessionBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = schedulersessionBL.Update(schedulersessionEntity);
        ...
        {
            var schedulersessionBL = new SchedulerSessionBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = schedulersessionBL.DeleteById(id);
                if (result.HasWarning())
                {
                    response.Message = String.Join(",", result.Warning);
                    return response;
