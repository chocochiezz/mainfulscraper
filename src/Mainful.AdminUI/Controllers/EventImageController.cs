...
{
    public class EventImageController : BaseController
    {
		[HttpPost]
        public override JsonResultEntity Get([FromBody] DBParamEntity dbParamEntity)
        {
            EventImageBL eventimageBL = new EventImageBL();
            JsonResultEntity response = new JsonResultEntity();

            try
            {
                var result = eventimageBL.GetAll(dbParamEntity);
                ...
                var dataFound = eventimageBL.GetTotalRows(dbParamEntity);
		...
		public JsonResultEntity GetById(int id)
        {
            ...
            try
            {
                var result = eventimageBL.GetById(id);
        ...
        [HttpPost]
		public JsonResultEntity Create([FromBody] EventImageEntity eventimageEntity)
        {
            EventImageBL eventimageBL = new EventImageBL();
            JsonResultEntity response = new JsonResultEntity();
            ...
            try
            {
                var result = eventimageBL.Create(eventimageEntity);
        ...
        [HttpPost]
        public JsonResultEntity Update([FromBody] EventImageEntity eventimageEntity)
        {
            EventImageBL eventimageBL = new EventImageBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = eventimageBL.Update(eventimageEntity);
        ...
        {
            var eventimageBL = new EventImageBL();
            JsonResultEntity response = new JsonResultEntity();
            try
            {
                var result = eventimageBL.DeleteById(id);
                if (result.HasWarning())
                {
                    response.Message = String.Join(",", result.Warning);
                    return response;
